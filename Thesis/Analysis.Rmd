...
The initital exploratory analysis shows clear trends, and the next phase of the analysis uses those trends and tests for significance using logistic regression analysis. 
...
logit.dem <- glm(URGCAREYes ~ INJURYYes + WEEKENDYES+ EstablishedPatient + PASTVIS + MAJORPreventative + MAJORRoutineChronic, data = dummyFull.varset, family = binomial)
...
logit.full <- glm(URGCAREYes ~ INJURYYes + WEEKENDYES+ EstablishedPatient + PASTVIS + MAJORPreventative + MAJORRoutineChronic + AGERWorkingAge + AGERRetired + SEXMale + RACERWhite + ZipHighIncome + URBANRURRural, data = dummyFull.varset, family = binomial)
...
logit2.full <- glm(PRIMCAREYes ~ INJURYYes + WEEKENDYES+ EstablishedPatient + PASTVIS + MAJORPreventative + MAJORRoutineChronic + AGERWorkingAge + AGERRetired + SEXMale + RACERWhite + ZipHighIncome + URBANRURRural, data = dummyUC.varset, family = binomial)
...
texreg(list(logit.beh, logit.dem, logit.full), 
       ...
       fontsize = "small", 
       custom.coef.names = c("Intercept", "Injury Related Visit", "Weekend Visit", "Established Patient", "Number Past Visits", "Visit Reason: Preventative", 
                             "Visit Reason: Routine/Chronic", "Age: Working", "Age: Retired", "Male", "Race: White", "High ZIP Income", "Rural"))
