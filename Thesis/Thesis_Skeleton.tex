...
\usepackage{natbib} \pagestyle{plain}
...
\begin{document}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# apparently this is super jargony??}
  \end{Highlighting}
  \end{Shaded}
  
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# I think the most e cient way to connect them more clearly is to prioritize the parts of your theoretical/historical narrative that explain the actions of patients.}
  
  \CommentTok{# Perhaps one thing to consider is moving the “Patient Choices” section beginning on page 11 to the beginning of the “Theoretical and Historical Foundations” chapter. If those arguments give the initial motivation for that chapter, they may also help you decide what needs more - and less - attention in the rest of the lit review.}
  
  \CommentTok{# Your lit review moves the reader toward thinking that urgent care is replacing primary care and the strong relationships patients have had with doctors. I get the sense that your assumption is that urgent care patients would have otherwise gone to primary care physicians or emergency departments. But these new urgent care facilities may also be attracting patients who would have otherwise not gone to any doctor. Is there anything theoretical to add about that possibility?}
  
  \CommentTok{# You discuss that patients have become increasingly demanding, attributing this to market-based theories about choice and beliefs about “shopping” for second options. Is there anything in the literature about other non-market explanations for patients becoming choosier? Speculating on whether things like WebMD or the popularity of media personalities like Dr Oz or Dr Phil have changed expectations about how people engage with medical institutions could be neat.}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# the research setup is that the hypotheses have urgent care facilities as the units of analysis but your models have patients as the units of analysis. I’ll repeat the same comment I had about the lit review: to make the connection between the hypotheses and your models more explicit, reframe them so they are from the perspective of patients. So, for H1 for example rather than “Urgent care centers will experience high levels of visits from uninsured and low SES patients” it could be something like “Uninsured and low SES patients will be more likely to visit an urgent care center than [the reference category]”.}
  
  \CommentTok{# One of your main theories is that urgent care centers solve the problem of overcrowded emergency departments. But it doesn’t look like you compare urgent care patients to emergency department patients directly. Why? That seems crucial to developing the argument that people are using urgent care centers for primary care.}
  
  \CommentTok{# Connected to the previous point, if the data allow it I would recommend replacing Table 2’s binary outcome with three outcomes: the decision to go to an urgent care center vs a primary care physician vs an emergency department. And for that setup, the preferable model would be a multinomial logistic regression. If that analysis is too advanced, you could do two separate logistic regressions: one comparing urgent care to primary care, and one comparing urgent care to emergency. But having all those comparisons in one model would be ideal.}
  \end{Highlighting}
  \end{Shaded}
  
  As mentioned previously, urgent care centers are an obviously
  understudied phenomenon, and the following section applies the
  sociological background discussed in the last chapter to better
  understand their role in the healthcare system. The conceptualization of
  urgent care as a new source of primary care for patients is contrasted
  against two other theories of urgent care center usage. The first
  applies theory to the widely matriculated explanation of urgent care by
  the media and health economists: that urgent care is a response to
  unavailability in primary care, developing out of consumer demand for
  quick and cost-efficient care. The second draws from the sociological
  research that focused on emergency departments (an oft cited cousin of
  urgent care centers), and explores the possibility that the industry has
  developed as a response to overcrowding and overuse of emergency
  departments in the past 20 years.
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# What is the age range?}
  
  \CommentTok{# You say race was recoded. . . but from what existing categories? And do you actually need four categories since 87% of your cases are white? It looks like the models eventually distinguish white from non-white. If race is binary, your list of independent variables should say that.}
  \CommentTok{# How is a “high zip income” defined? What is the dollar cuto ?ff Is this variable the same as “Percent Poverty” in Appendix A?}
  
  \CommentTok{# The urban/rural dummy you describe and have in your models does not match the more specific categories in Figure 1 (large metro vs rural vs medium metro vs large fringe metro) and the Appendix A table (which also includes “small metro”). As you revise, make sure the reader can find a description of every variable you show.}
  \end{Highlighting}
  \end{Shaded}
